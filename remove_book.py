...
self.iconbitmap(r'libico.ico') self.configure(bg='tan2') self.maxsize(500,250) self.minsize(500,250) self.title("Remove book")
a = StringVar()


def aaa():
if len(a.get()) == 0:
messagebox.showerror("Error","Please Enter The Book Id") else:
...
if d:
self.destroy()
os.system('%s %s' % (py, 'remove_book.py')) else:
self.destroy() except Error:
messagebox.showerror("Error", "Something Goes Wrong")


lb = Label(self, text="Enter Book Id", font=('Comic Scan Ms', 20, 'bold'), bg="tan2")
lb.place(x=30, y=70)
e = Entry(self, textvariable=a, width=30).place(x=240, y=75) bt = Button(self, text="Remove", width=15, command=aaa,
background="burlywood3", font=('Arial', 10, 'bold')).place(x=240, y=120)
